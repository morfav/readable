...
export const getComparator = (type, isAscending, objectsToSort) => {
  ...
  if (isAscending) {
    return ((postOne, postTwo) =>
      (postOne[type] - postTwo[type] === 0
        ? objectsToSort.indexOf(postOne) - objectsToSort.indexOf(postTwo)
        : postOne[type] - postTwo[type]));
  }
  return ((postOne, postTwo) =>
    (postTwo[type] - postOne[type] === 0
      ? objectsToSort.indexOf(postOne) - objectsToSort.indexOf(postTwo)
      : postTwo[type] - postOne[type]));
};

export const getIcon = (type, isAscending, postsComparator, postId) => {
  let icon = null;
  if (postsComparator === type && !postId) {
    icon = isAscending ? <ArrowDropUp /> : <ArrowDropDown />;
  }
  return icon;
};
